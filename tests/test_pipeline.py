...
class PipelineInputTests(unittest.TestCase):
    ...
    def test_validate_portfolio_accepts_matching_profiles(self):
        pipeline.validate_input(self.valid_portfolio_payload())
        self.assertEqual(pipeline.input_mode(self.valid_portfolio_payload()), "portfolio")
...
class OrchestrationTests(unittest.TestCase):
    def make_tool(self, root, name, code):
        ...
        path = directory / "main.py"; path.write_text(textwrap.dedent(code), encoding="utf-8"); return path
    ...
    def test_end_to_end_orchestration_with_tool_contracts(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            root=Path(tmpdir); self.install_tools(root); tools=pipeline.resolve_tools(root)
            scorecard_main=Path(__file__).resolve().parents[1]/"main.py"; work=root/"work"
    ...
    def test_end_to_end_portfolio_ranks_all_suppliers(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn('explanation', result)
            self.assertEqual(result['explanation']['winner'],'Supplier A')
            self.assertEqual(result['explanation']['runner_up'],'Supplier B')
            self.assertTrue(result['explanation']['advantages'])
