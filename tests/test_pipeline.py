...
class PipelineInputTests(unittest.TestCase):
    ...
    def test_validate_input_requires_target_supplier_in_quotes(self):
        ...
        with self.assertRaisesRegex(
            ValueError,
            "do not contain target supplier",
        ):
...
class OrchestrationTests(unittest.TestCase):
    ...
    def install_tools(self, root):
        self.make_tool(
            ...
            p.add_argument('--target-currency')
            p.add_argument('--output')
            a=p.parse_args()
            q=json.loads(Path(a.quote).read_text())
            q['currency']=a.target_currency
        ...
        self.make_tool(
            ...
            from pathlib import Path
            p=argparse.ArgumentParser()
            p.add_argument('quotes', nargs='+')
            p.add_argument('--output')
            a=p.parse_args()
            qs=[json.loads(Path(x).read_text()) for x in a.quotes]
            for q in qs:
                q['score']=92.0 if q['name']=='Supplier A' else 96.0
        ...
        self.make_tool(
            ...
            p.add_argument('--on-time-delivery')
            p.add_argument('--defect-rate')
            p.add_argument('--prepayment-exposure')
            p.add_argument('--compliance-incidents')
            p.add_argument('--dependency-share')
    ...
    def test_single_supplier_compliance_block_overrides_score(self):
        payload = PipelineInputTests().valid_payload()
        payload["vendor_risk"]["compliance_incidents"] = 3
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            self.install_tools(root)
            result = pipeline.run_pipeline(
    ...
    def test_end_to_end_portfolio_ranks_all_suppliers(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(
                result["tool"],
                "supplier-scorecard-portfolio",
            )
            self.assertEqual(result["supplier_count"], 2)
            self.assertEqual(result["version"], "0.3")
            self.assertEqual(
                result["orchestration"]["version"],
                "0.4",
            )
            self.assertEqual(
                [x["rank"] for x in result["suppliers"]],
                [1, 2],
            )
            self.assertIn("policy_decision", result)
            self.assertTrue(
                (work / "suppliers" / "Supplier-A" / "payment.json").exists()
            )
    ...
    def test_policy_can_select_lower_scoring_pass_supplier(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            b = next(
                x for x in result["suppliers"]
                if x["supplier"] == "Supplier B"
            )
            self.assertEqual(
                b["policy"]["status"],
                "REVIEW",
            ...
            self.assertIn(
                "not auto-eligible",
                result["policy_decision"]["summary"],
            )

    def test_blocked_top_supplier_is_never_auto_recommended(self):
        payload = PipelineInputTests().valid_portfolio_payload()
        payload["supplier_profiles"][1]["payment_terms"] = "Net 30 days"
        payload["supplier_profiles"][1]["vendor_risk"][
    ...
    def test_no_auto_eligible_supplier_withholds_recommendation(self):
        payload = PipelineInputTests().valid_portfolio_payload()
        payload["supplier_profiles"][0]["vendor_risk"][
            "compliance_incidents"
        ] = 1
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
