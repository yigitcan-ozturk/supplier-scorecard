...
REQUIRED_TOOLS = ("currency-normalizer", "rfqdiff", "payment-terms-parser", "vendor-risk-engine")
QUOTE_FIELDS = ("name", "currency", "price", "lead_time_weeks", "payment_days")
VENDOR_FIELDS = ("on_time_delivery", "defect_rate", "compliance_incidents", "dependency_share")
...
def load_input(path, *, profile_file=None):
    ...
    if profile_file is not None:
        ...
        payload["profile_file"] = str((custom if custom.is_absolute() else path.parent / custom).resolve())
...
def _validate_vendor(vendor, context):
...
def validate_input(payload):
    ...
    resolve_profile(payload.get("category_profile"), policy=payload.get("policy"), profile_file=payload.get("profile_file"))
    target = str(payload["target_currency"]).strip().upper()
    if len(target) != 3:
    ...
    for index, quote in enumerate(quotes, 1):
        ...
        if not name or _key(name) in quote_names:
            raise ValueError(f"invalid or duplicate quotation supplier: '{name}'.")
        quote_names[_key(name)] = name
    ...
    if input_mode(payload) == "single":
        ...
        _validate_vendor(payload["vendor_risk"], "vendor_risk")
...
def resolve_tools(tools_root):
    ...
    for name in REQUIRED_TOOLS:
        ...
        if not path.is_file():
            raise ValueError(f"missing tool '{name}': expected {path}.")
        tools[name] = path
    return tools


def run_command(command, *, capture_json=False):
    process = subprocess.run([str(x) for x in command], capture_output=True, text=True, check=False)
...
def _prepare_rfq(payload, tools, work_dir):
    normalized = []
    for index, quote in enumerate(payload["quotes"], 1):
        raw, out = work_dir / f"quote-{index}-raw.json", work_dir / f"quote-{index}-normalized.json"
        write_json(quote, raw)
        run_command([sys.executable, tools["currency-normalizer"], "--quote", raw, "--target-currency", payload["target_currency"], "--output", out])
...
def _run_supplier(profile, tools, scorecard_main, rfq_path, supplier_dir, *, resolved_profile):
    ...
    vendor_payload = run_command([
        ...
        "--on-time-delivery", vendor["on_time_delivery"], "--defect-rate", vendor["defect_rate"],
        "--prepayment-exposure", commercial, "--compliance-incidents", vendor["compliance_incidents"],
        "--dependency-share", vendor["dependency_share"], "--json",
    ], capture_json=True)
    vendor_path = supplier_dir / "vendor-risk.json"
    write_json(vendor_payload, vendor_path)
    command = [sys.executable, scorecard_main, supplier, "--rfq-json", rfq_path, "--payment-json", payment, "--vendor-risk-json", vendor_path]
    if profile.get("technical_compliance") is not None:
        command += ["--technical-compliance", profile["technical_compliance"]]
    if resolved_profile["source"]["type"] == "file":
        command += ["--profile-file", resolved_profile["source"]["path"]]
    else:
        command += ["--category-profile", resolved_profile["name"]]
    command += ["--json"]
    result = run_command(command, capture_json=True)
    result = apply_policy(result, compliance_incidents=vendor["compliance_incidents"], policy=resolved_profile["policy"])
    return result, payment, vendor_path
...
def _policy_selection(ranked):
    ...
    if recommended is None:
    ...
    elif recommended is leader:
        summary = f"{leader['supplier']} is the highest-scoring supplier and passes all automatic policy gates."
    else:
        ...
        summary = f"{leader['supplier']} is the score leader at {leader['score']:.2f}/100 but is not auto-eligible because {reason[0].lower()+reason[1:]} {recommended['supplier']} is the highest-scoring supplier that passes all automatic gates."
    return {
...
def run_pipeline(payload, tools, scorecard_main, work_dir):
    ...
    resolved = resolve_profile(payload.get("category_profile"), policy=payload.get("policy"), profile_file=payload.get("profile_file"))
    rfq = _prepare_rfq(payload, tools, work_dir)

    if input_mode(payload) == "single":
        profile = {"supplier": payload["supplier"], "payment_terms": payload["payment_terms"], "vendor_risk": payload["vendor_risk"], "technical_compliance": payload.get("technical_compliance")}
        result, payment, vendor = _run_supplier(profile, tools, scorecard_main, rfq, work_dir, resolved_profile=resolved)
        result["orchestration"] = {
            "tool": "supplier-scorecard-pipeline", "version": PIPELINE_VERSION, "mode": "single",
            "target_currency": payload["target_currency"], "category_profile": resolved["name"], "profile_source": resolved["source"],
            "artifacts": {"rfq": str(rfq), "payment": str(payment), "vendor_risk": str(vendor)},
        }
        return result

    results, artifact_map, used = [], {}, set()
    root = work_dir / "suppliers"
    for index, profile in enumerate(payload["supplier_profiles"], 1):
        supplier, dirname = str(profile["supplier"]).strip(), _safe_dir(profile["supplier"])
        if dirname.casefold() in used:
            dirname = f"{dirname}-{index}"
        used.add(dirname.casefold())
        result, payment, vendor = _run_supplier(profile, tools, scorecard_main, rfq, root / dirname, resolved_profile=resolved)
        results.append(result)
        artifact_map[supplier] = {"payment": str(payment), "vendor_risk": str(vendor)}
    ranked = _rank(results)
    decision = _policy_selection(ranked)
    return {
        "tool": "supplier-scorecard-portfolio", "version": "1.0",
        "category_profile": resolved["name"],
        "profile": {"name": resolved["name"], "description": resolved["description"], "weights": resolved["weights"], "policy": resolved["policy"], "source": resolved["source"]},
        "recommended_supplier": decision["recommended_supplier"], "top_scoring_supplier": decision["top_scoring_supplier"], "decision_status": decision["status"],
        "supplier_count": len(ranked), "target_currency": payload["target_currency"], "policy": resolved["policy"], "suppliers": ranked,
        "explanation": explain_portfolio(ranked), "policy_decision": decision,
        "orchestration": {"tool": "supplier-scorecard-pipeline", "version": PIPELINE_VERSION, "mode": "portfolio", "target_currency": payload["target_currency"], "category_profile": resolved["name"], "profile_source": resolved["source"], "artifacts": {"rfq": str(rfq), "supplier_outputs": artifact_map}},
    }
...
def print_result(result):
    ...
    if mode == "portfolio":
        ...
        for item in result["suppliers"]:
            print(f"{item['rank']:>2} {item['supplier']:<28} {item['score']:>7.2f} {item['final_decision']}")
        ...
        print(f"Decision reason      : {result['policy_decision']['summary']}")
    else:
...
def _temporary_artifacts(result):
    result["orchestration"]["artifacts"] = {"rfq": "temporary", "supplier_outputs": "temporary"} if result["orchestration"]["mode"] == "portfolio" else {"rfq": "temporary", "payment": "temporary", "vendor_risk": "temporary"}
    ...
    try:
        ...
        if args.work_dir:
            ...
            with tempfile.TemporaryDirectory(prefix="supplier-scorecard-") as temp:
                result = run_pipeline(payload, tools, scorecard_dir / "main.py", temp)
                _temporary_artifacts(result)
        if args.output:
            write_json(result, args.output)
    except (OSError, json.JSONDecodeError, TypeError, ValueError, RuntimeError) as exc:
        parser.error(str(exc))
    print(json.dumps(result, indent=2, ensure_ascii=False) if args.json else "", end="" if args.json else "")
    if not args.json:
        print_result(result)
